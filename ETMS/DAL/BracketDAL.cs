...
namespace ETMS.DAL
{
    public class BracketDAL
    {
        ...
        public bool SaveBracketWithRefs(List<MatchDTO> matches)
        {
            ...
            try
            {
                // Bước 1: INSERT tất cả match (chưa có NextMatchID vì chưa có ID)
                ...
                foreach (var m in matches)
                {
                    ...
                    cmd.Parameters.AddWithValue("@win",     (object?)m.WinnerID ?? DBNull.Value);
                    cmd.Parameters.AddWithValue("@st",      m.Status);
                    cmd.Parameters.AddWithValue("@schTime", (object?)m.ScheduledTime ?? DBNull.Value);
                    cmd.Parameters.AddWithValue("@rnd",     m.Round);
                    cmd.Parameters.AddWithValue("@ord",     m.MatchOrder);
                    cmd.Parameters.AddWithValue("@bye",     m.IsBye ? 1 : 0);
                    int newID = Convert.ToInt32(cmd.ExecuteScalar());
                    insertedIDs.Add(newID);
                    m.MatchID = newID;   // ghi lại MatchID thực tế
                }

                // Bước 2: UPDATE NextMatchID dựa trên ánh xạ Round → Match
                // matches đã được BracketBUS sắp xếp — NextMatchID được set dựa trên index
                const string updateSql = @"
                    UPDATE tblMatch
                    SET NextMatchID = @nxt, NextMatchSlot = @slot
                    WHERE MatchID = @id";
                ...
                foreach (var m in matches)
                {
                    ...
                    {
                        using var cmd = new SqlCommand(updateSql, conn, trans);
                        cmd.Parameters.AddWithValue("@nxt",  m.NextMatchID.Value);
                        cmd.Parameters.AddWithValue("@slot", (object?)m.NextMatchSlot ?? DBNull.Value);
                        cmd.Parameters.AddWithValue("@id",   m.MatchID);
                        cmd.ExecuteNonQuery();
                    }
                }
