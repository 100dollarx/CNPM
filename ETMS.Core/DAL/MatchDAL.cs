    ...
    public class MatchDAL
    {
        ...
        public bool SetWinnerAndAdvance(int matchID, int winnerID, int loserID, string status = "Completed")
        {
            ...
            try
            {
                ...
                // Fix B-2: Chỉ set ActualStartTime nếu là trận thực (Completed), không set khi Walkover
                string updateMatchSql = status == "Completed"
                    ? "UPDATE tblMatch SET WinnerID=@win, LoserID=@lose, Status=@st, ActualStartTime=GETDATE() WHERE MatchID=@id"
                    : "UPDATE tblMatch SET WinnerID=@win, LoserID=@lose, Status=@st WHERE MatchID=@id";
                using (var cmd = new SqlCommand(updateMatchSql, conn, trans))
                {
