...
namespace ETMS.BUS
{
    ...
    public class BracketBUS
    {
        ...
        public (bool success, string message) GenerateBracket(int tournamentID)
        {
            ...
            // ----- Vòng 2 trở đi -----
            // Fix B-3: Tính matchesThisRound từ slots/(2^round), KHÔNG dùng prevRound.Count/2
            // vì vòng 1 có thể gồm bye + real matches lẫn lộn khiến Count sai.
            for (int r = 2; r <= rounds; r++)
            {
                var currentRound = new List<MatchDTO>();
                int matchesThisRound = slots / (int)Math.Pow(2, r);
